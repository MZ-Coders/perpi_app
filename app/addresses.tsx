...
export default function AddressesScreen() {
  ...
  // Atualizar a lista quando a tela ganhar foco (quando voltar do formulário)
  useFocusEffect(
    useCallback(() => {
      console.log('Focus effect triggered, fetching addresses...');
      setRefreshKey(prev => prev + 1); // Força re-render
      fetchAddresses(false); // Não mostrar loading quando voltar do formulário
    }, [fetchAddresses])
  );
  ...
  if (loading) {
    return (
      <View style={styles.container}>
        <AppHeader title="Meus Endereços" backMode={true} onMenuPress={() => router.back()} />
